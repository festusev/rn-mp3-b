...
export default function FeedScreen({ navigation }: Props) {
  ...
  const renderSocial = ({ item }: { item: SocialModel }) => {
    ...
    const deleteSocial = () => {
      firebase.firestore().collection("socials").doc(item.id).delete().then(() => {
    ...
    const toggleLikeSocial = () => {
      if(item.liked.includes(curUID)) {
        let index = item.liked.indexOf(curUID);
        item.liked.splice(index,1);
      } else {
        item.liked.push(curUID);
      }
      firebase.firestore().collection("socials").doc(item.id).update({liked: item.liked});
    }


    let deleteBtn = <></>;
    if(curUID == item.uid) {
      deleteBtn = <Button onPress={deleteSocial}>Delete</Button>
    }

    let likeBtn = <Button onPress={toggleLikeSocial}>{item.liked.includes(curUID) ? "Liked": "Like"}</Button>;
